
<footer style="clear: both">
	<div id="teapopFooterTop">
		<div id="teapopInformation">
			<div id="siteInformation">SITE INFORMATION</div>

			<ul class="siteInformationLinks">
				<li class="pointer" onclick="javascript:showDialog()">About Us</li>
				<li class="pointer" onclick="javascript:showDialog()">Terms of Service</li>
				<li class="pointer" onclick="window.location.href='admin'">Admin Site</li>
			</ul>
		</div>


		<div id="teapopJoinUs">
			<div id="siteInformation">TEA-POP MALABON (MAIN)</div>
			<ul class="siteInformationLinks">
				<li>Cenel Bldg. Burgos Concepcion, 1478 Malabon City</li>
				<li>376-4323</li>
			</ul>
		</div>

		<div id="teapopJoinUs">
			<div id="siteInformation">TEA-POP NAVOTAS</div>
			<ul class="siteInformationLinks">
				<li>M Naval St C4 Road Bagumbayan North Navotas City</li>
				<li>376-4094</li>
			</ul>
		</div>
	</div>

	<div id="teapopFooterBottom">
		<div id="copyright">
			<center>
				<a href="#">UPOU-CRUX</a> &#169; 2014 All Rights Reserved. <br>
				Version 1.0.0 <br>
				| PATRICK DUREZA | MARY JOY PASCUAL | TIM TAN |<br>
			</center>
		</div>
	</div>
	<div class="clearfix"></div>
	<div id="aboutus-dialog" title="Page is Under Construction" style="display: none; height: 200px;">
 		 <p>Sorry, this feature is still under construction.</p>
	</div>
</footer>

